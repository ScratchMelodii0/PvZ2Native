...
public class Facebook {
    ...
    private void authorize(Activity activity, String[] permissions, int activityCode, SessionLoginBehavior behavior, final DialogListener listener) {
        ...
        Session.OpenRequest openRequest = new Session.OpenRequest(activity).setCallback(callback).setLoginBehavior(behavior).setRequestCode(activityCode).setPermissions(Arrays.asList(permissions));
    ...
    public void onSessionCallback(Session callbackSession, SessionState state, Exception exception, DialogListener listener) {
        ...
        if (exception != null) {
            ...
            if ((exception instanceof FacebookAuthorizationException) && extras != null && extras.containsKey(Session.WEB_VIEW_ERROR_CODE_KEY) && extras.containsKey(Session.WEB_VIEW_FAILING_URL_KEY)) {
    ...
    public void dialog(Context context, String action, Bundle parameters, DialogListener listener) {
        ...
        if (action.equals(LOGIN)) {
            ...
            parameters.putString(ServerProtocol.DIALOG_PARAM_APP_ID, this.mAppId);
            if (isSessionValid()) {
