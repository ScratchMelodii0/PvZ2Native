...
public class IabHelper {
    ...
    public void startSetup(final OnIabSetupFinishedListener listener) {
        ...
        this.mServiceConn = new ServiceConnection() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.1
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                if (!IabHelper.this.mDisposed) {
                    ...
                    try {
                        ...
                        int response2 = IabHelper.this.mService.isBillingSupported(3, packageName, "subs");
                        if (response2 == 0) {
    ...
    public void dispose() {
        if (this.mAsyncInProgress) {
            this.mDisposeRequested = true;
            return;
        }
        logDebug("Disposing.");
        this.mDisposeRequested = false;
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == 0) {
            if (data != null) {
                logDebug("Purchase canceled - Response: " + getResponseDesc(getResponseCodeFromIntent(data)));
            } else {
                logDebug("Purchase canceled - Response: null");
            }
            IabResult result = new IabResult(IABHELPER_USER_CANCELLED, "User canceled.");
        ...
        if (data == null) {
            ...
            IabResult result2 = new IabResult(IABHELPER_BAD_RESPONSE, "Null data in IAB result");
            if (this.mPurchaseListener != null) {
        ...
        if (resultCode == -1 && responseCode == 0) {
            ...
            if (purchaseData == null || dataSignature == null) {
                ...
                IabResult result3 = new IabResult(IABHELPER_UNKNOWN_ERROR, "IAB returned null purchaseData or dataSignature");
                if (this.mPurchaseListener != null) {
            ...
            try {
                ...
                try {
                    ...
                    if (!Security.verifyPurchase(this.mSignatureBase64, purchaseData, dataSignature)) {
                        ...
                        IabResult result4 = new IabResult(IABHELPER_VERIFICATION_FAILED, "Signature verification failed for sku " + sku);
                        if (this.mPurchaseListener != null) {
                            this.mPurchaseListener.onIabPurchaseFinished(result4, purchase);
                        }
                        return true;
                    }
                    ...
                    e.printStackTrace();
                    IabResult result5 = new IabResult(IABHELPER_BAD_RESPONSE, "Failed to parse purchase data.");
                    if (this.mPurchaseListener != null) {
            ...
            if (this.mPurchaseListener != null) {
                IabResult result6 = new IabResult(responseCode, "Problem purchashing item.");
                this.mPurchaseListener.onIabPurchaseFinished(result6, null);
            }
        } else if (resultCode == 0) {
            logDebug("Purchase canceled - Response: " + getResponseDesc(responseCode));
            IabResult result7 = new IabResult(IABHELPER_USER_CANCELLED, "User canceled.");
            if (this.mPurchaseListener != null) {
            ...
            IabResult result8 = new IabResult(IABHELPER_UNKNOWN_PURCHASE_RESPONSE, "Unknown purchase response.");
            if (this.mPurchaseListener != null) {
                this.mPurchaseListener.onIabPurchaseFinished(result8, null);
            }
        }
        return true;
    }
    ...
    public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus, final QueryInventoryFinishedListener listener) {
        ...
        new Thread(new Runnable() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.2
            ...
            public void run() {
                ...
                if (!IabHelper.this.mDisposed && listener != null) {
                    handler.post(new Runnable() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.2.1
                        @Override // java.lang.Runnable
                        public void run() {
    ...
    int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws JSONException, RemoteException {
        ...
        ArrayList<String>[] partitionedSkus = new ArrayList[(skuList.size() / 20) + 1];
        logDebug("requesting a total of " + skuList.size() + " skus");
        logDebug("partitionedSkus length " + partitionedSkus.length);
        for (int i = 0; i < partitionedSkus.length; i++) {
            partitionedSkus[i] = new ArrayList<>();
        }
        String[] skuListStr = (String[]) skuList.toArray(new String[skuList.size()]);
        logDebug("skuList.size() " + skuList.size());
        logDebug("skuListStr.length " + skuListStr.length);
        for (int i2 = 0; i2 < skuListStr.length; i2++) {
            partitionedSkus[i2 / 20].add(skuListStr[i2]);
        }
        Bundle querySkus = new Bundle();
        for (ArrayList<String> skus : partitionedSkus) {
    ...
    void consumeAsyncInternal(final List<Purchase> purchases, final OnConsumeFinishedListener singleListener, final OnConsumeMultiFinishedListener multiListener) {
        ...
        new Thread(new Runnable() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.3
            @Override // java.lang.Runnable
            public void run() {
                ...
                if (!IabHelper.this.mDisposed && singleListener != null) {
                    handler.post(new Runnable() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.3.1
                        @Override // java.lang.Runnable
                        public void run() {
                ...
                if (!IabHelper.this.mDisposed && multiListener != null) {
                    handler.post(new Runnable() { // from class: com.popcap.SexyAppFramework.purchase.IabHelper.3.2
                        @Override // java.lang.Runnable
                        public void run() {
